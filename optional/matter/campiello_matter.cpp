...
public:
	...
	MatterWindow(const std::string& host, const std::string& name, const MatterInfo& info)
		: BWindow(BRect(100, 100, 470, 420), "Dispositivo Matter", B_TITLED_WINDOW,
			B_NOT_ZOOMABLE | B_AUTO_UPDATE_SIZE_LIMITS)
	{
		...
		if (!info.deviceName.empty())
			s << "Nome: " << info.deviceName.c_str() << "\n";
		if (info.deviceTypeId > 0)
			s << "Tipo: " << info.deviceType.c_str() << " (id " << info.deviceTypeId << ")\n";
		if (info.vendorId > 0)
			s << "Vendor / Product: " << info.vendorId << " / " << info.productId << "\n";
		if (!info.commissioningText.empty())
			s << "Stato: " << info.commissioningText.c_str() << "\n";
		if (!info.discriminator.empty())
			s << "Discriminator: " << info.discriminator.c_str() << "\n";
		s << "Trasporto: " << (info.tcpSupported ? "TCP + UDP" : "UDP") << "\n";
		s << "Indirizzo: " << host.c_str() << "\n";
		if (s.Length() == 0)
			s << "Dispositivo Matter.\nIndirizzo: " << host.c_str() << "\n";
